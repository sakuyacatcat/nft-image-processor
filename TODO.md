...
- [ ] 入力された paletteIndex に対応するカラーパレットに color を保存できる
  - [ ] 入力された paletteIndex が 0~255 の整数であり、color が 6 桁の 16 進数文字列の場合、カラーパレットへの保存に成功する
  - [ ] 入力された paletteIndex が 0~255 の整数であるが、color が 6 桁の 16 進数文字列でない場合、カラーパレットへの保存に失敗する
  - [ ] 入力された paletteIndex が 0~255 の整数以外の場合、カラーパレットへの保存に失敗する
