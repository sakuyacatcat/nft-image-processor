...
- ユーザーは WebUI 上でドット絵の svg を入力して NFT 発行できる
  - (テストしない)mint 関数を呼び出せる
  - (テスト)svg 文字列を入力に svg オブジェクトを作成できる
...
- [ ] svg 文字列を入力に svg オブジェクトを作成できる
  - [ ] 入力された svg 文字列の適正/不正を判定できる
  - [ ] 適正な svg 文字列の入力から、svg オブジェクトの作成に成功する
  - [ ] 不正な svg 文字列の入力から、svg オブジェクトの作成に失敗する
